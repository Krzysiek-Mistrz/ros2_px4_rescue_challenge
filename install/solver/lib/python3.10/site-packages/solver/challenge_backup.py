...
class SolverClass(Node):
    ...
    def camera_callback(self, msg):
        ...
        if frame is None or frame.size == 0:
            ...
            return
        # w tym miejscu należy wywołać algorytm wizyjny na frameie i w razie wykrycia nowego obiektu przypisać jego pozycje do bazy danych
        #debug
        #self.get_logger().info(f"Frame height = {frame.shape[0]}, frame width = {frame.shape[1]}")
        if self.uav_position and self.currnet_target_point:
    ...
    def detect_objects(self, frame, threshold = 200):
        ...
        # Przekształć obraz na przestrzeń kolorów RGB (OpenCV domyślnie używa BGR)
        image_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        # Definicja progów dla każdego kanału (odpowiednik MATLAB)
        channel1_min, channel1_max = 86, 255
        channel2_min, channel2_max = 0, 75
        channel3_min, channel3_max = 0, 94

        # Utwórz maskę dla koloru czerwonego
        mask = (
        ...
        # Utwórz obraz binarny: czerwone piksele -> białe, reszta -> czarne
    ...
    def check_and_add_to_detected(self, px: float, py: float, center_of_frame):
        '''Funkcja licząca położnie zidentyfikowanego obiektu
        Zakładając że uav leci na stałej wyskości wiadomo będzie ilu metrom równy jest jeden piksel
        (jak uav będzie zmieniał wyskokośc lotu to trzeba zrobic lookuptable z takim przelicznikiem, lub jakaś zależność typu regresja liniowa)
        Na podstawie informacji o środku frameu, położniu środka zidentyfikowanego obiektu (px,py) i informacji o położneiu uav'a
        wyliczamy rzeczywiste położenie obiektu
        
        Sprawdzenie czy obiekt jest już w bazie znalezionych
        '''

        # współczynniki
        a_hor = -4.163469105855614; b_hor = 75.2155798055519
        a_ver = -6.245217604480979; b_ver = 112.8236216457635

        uav_x, uav_y, h = self.uav_position
        h = -h
        # przeliczniki z matrów na pixele zależne od wysokości
        ...
        obj_x, obj_y = uav_x + pixels_to_meters_horizontal * abs(center_of_frame[0] - px), uav_y + pixels_to_meters_vertical * abs(center_of_frame[1] - px)

        for prev_x, prev_y, _ in self.detected_objects_positions:
            dist = sqrt( (obj_x - prev_x)**2 + (obj_y - prev_y)**2 )
            if dist > self.min_dist:
                self.detected_objects_positions.append((prev_x, prev_y, 0))
    ...
    def report_object_locations(self, positions):
        ...
        self.get_logger().info("DANO POZYCJE!!!!!!!!!!!!!!!!!!!!!!")
